...
std::shared_ptr<lynx::lepus::Context> GetVMContent(
    const EncoderOptions& encoder_options, lepus::ContextType ctx_type) {
  std::shared_ptr<lynx::lepus::Context> vm_context(
      lynx::lepus::Context::CreateContext(ctx_type));
  ...
  if (vm_context->IsLepusNGContext()) {
    lynx::lepus::QuickContext* quick_ctx =
        lynx::lepus::Context::ToQuickContext(vm_context.get());
  ...
  } else if (vm_context->IsVMContext()) {
...
std::unique_ptr<TemplateBinaryWriter> EncodeTemplate(
  ...
  lepus::ContextType ctx_type = GetContextType(encoder_options);
  auto encoder = std::make_unique<TemplateBinaryWriter>(
      vm_context ? vm_context->GetMTSContext() : nullptr, ctx_type,
...
lynx::tasm::EncodeResult EncodeInner(const std::string& options_str) {
  ...
  lepus::ContextType ctx_type = GetContextType(encoder_options);
  std::shared_ptr<lynx::lepus::Context> vm_context;
  // RTS binary is generated by the frontend CLI.
  if (ctx_type == lepus::ContextType::VMContextType ||
      ctx_type == lepus::ContextType::LepusNGContextType) {
...
lynx::tasm::EncodeResult encode_ssr_inner(const uint8_t* ptr, size_t buf_len,
  ...
  std::shared_ptr<lynx::lepus::Context> vm_context(
      lynx::lepus::Context::CreateContext(lepus::ContextType::VMContextType));
...
const char* lepusCheck(const char* sourceFile, char* targetSdkVersion,
  ...
  std::shared_ptr<lynx::lepus::Context> context =
      lynx::lepus::Context::CreateContext(
          lynx::lepus::ContextType::VMContextType);
  context->Initialize();
  ...
  lynx::tasm::Renderer::RegisterBuiltin(context.get(),
                                        lynx::tasm::ArchOption::RADON_ARCH);
  std::string err_str = lynx::lepus::BytecodeGenerator::GenerateBytecode(
      context->GetMTSContext(), sourceFile, targetSdkVersion);
  if (err_str.length() > 0) {
